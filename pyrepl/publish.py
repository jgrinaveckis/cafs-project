...
def insert_row(connection, data: dict):
    try:
        ...
        cursor.execute(query, (data["ip"], data["region"], data["country_code"],
                       data["country_name"], data["city"], data["lat"], data["lon"], timestamp, data["table"]))
        connection.commit()
...
def build_message(binlog_event, row):
    ...
    if isinstance(binlog_event, WriteRowsEvent) and 'lead_insert' == tbl_name:
...
def send_event():
    ...
    for event in stream:
        for e_row in event.rows:
            ...
            if msg:
                logging.info(
                    f"Table: {msg['table']} received {msg['event']} type of change")
